    ...
    <div className="animate-in fade-in duration-500 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-[#A3AED0] font-medium mt-1">Quản lý danh sách, logo và trạng thái hoạt động của các thương hiệu</p>
        </div>
      ...
      <div className="bg-[#FFFFFF] rounded-md overflow-hidden mb-8 border border-[#E0E5F2]">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#F8FAFC]">
              <tr className="border-b border-[#E0E5F2]">
                <th className="px-6 py-4 text-[12px] font-bold text-[#A3AED0] uppercase">Thương hiệu</th>
                <th className="px-6 py-4 text-[12px] font-bold text-[#A3AED0] uppercase text-center">Mã (BrandCode)</th>
            ...
            <tbody className={`text-sm bg-white transition-opacity duration-200 ${loading && filteredBrands.length > 0 ? 'opacity-60 pointer-events-none' : ''}`}>
              {loading && filteredBrands.length === 0 ? (
                [...Array(pageSize)].map((_, idx) => (
                  <tr key={idx} className="border-b border-[#E0E5F2] h-[68px]">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-md bg-neutral-100 animate-pulse border border-[#E0E5F2]"></div>
                ...
                filteredBrands.map((brand) => {
                    ...
                    return (
                      <React.Fragment key={brand.id}>
                        {/* Main Row */}
                        <tr className={`hover:bg-[#F4F7FE] transition-all group border-b border-[#E0E5F2] ${!isBrandActive ? 'opacity-50 grayscale bg-gray-50/50' : ''}`}>
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                {brand.description && (
                                  <span className="block text-xs text-[#A3AED0] max-w-xs truncate mt-0.5">{brand.description}</span>
                                )}
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4 text-center">
                            <span className="text-sm font-semibold text-[#4318FF] bg-[#F4F7FE] px-3 py-1 rounded-md">
                              {brand.brandCode}
                            </span>
                          </td>
                          ...
                          <td className="px-6 py-4 text-center">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <button
                                ...
                                className={`p-2 rounded-md transition-all ${isExpanded ? 'bg-[#4318FF] text-white' : 'text-[#A3AED0] hover:text-[#4318FF] hover:bg-[#F4F7FE]'}`}
                        ...
                        {isExpanded && (
                          <tr className="bg-[#F8FAFC]/40">
                            <td colSpan="5" className="p-0 border-b border-[#E0E5F2]">
                              <div className="px-12 py-5 border-l-4 border-[#4318FF]/30 bg-gray-50/20">
                                {statsLoading ? (
                                  <div className="flex justify-center items-center py-6">
                                    <Loader2 size={24} className="animate-spin text-[#4318FF]" />
                                    <span className="text-sm font-semibold text-[#2B3674] ml-2">Đang tải thống kê nhanh...</span>
                                  </div>
                                ) : (brand.productsCount === 0 || !stats || (stats.totalActive === 0 && stats.totalStock === 0)) ? (
                                  /* Empty State Layout */
                                  <div className="flex items-center justify-between h-16 px-6 bg-white rounded-md border border-dashed border-[#E0E5F2]">
                                    <div className="flex items-center text-[#A3AED0] gap-2">
                                      <FolderOpen size={20} className="text-[#A3AED0] opacity-50" />
                                      <span className="text-sm font-semibold">Chưa có sản phẩm nào thuộc thương hiệu này.</span>
                                    </div>
                                    <button 
                                      type="button" 
                                      className="flex items-center gap-1.5 px-4 py-2 bg-[#F4F7FE] text-[#4318FF] hover:bg-[#EBF4FF] rounded-md text-xs font-bold transition-all active:scale-[0.98]"
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-3 bg-neutral-50/80 rounded-md border border-neutral-100 p-5 divide-y md:divide-y-0 md:divide-x divide-gray-200 animate-in slide-in-from-top-2 duration-300">
                                    ...
                                    <div className="pb-4 md:pb-0 md:pr-6 flex flex-col justify-center">
                                      <h5 className="text-xs font-extrabold text-[#A3AED0] uppercase tracking-wider mb-3">Hiệu suất (Performance)</h5>
                                      <div className="space-y-2">
                                        <div className="flex justify-between items-center text-sm font-medium">
                                          <span className="text-[#2B3674] flex items-center gap-2">
                                            <span className="w-2.5 h-2.5 rounded-full bg-[#01B574] inline-block"></span>
                                            Đang bán:
                                          </span>
                                          <span className="text-[#01B574] font-bold">{stats.totalActive} SP</span>
                                        </div>
                                        <div className="flex justify-between items-center text-sm font-medium">
                                          <span className="text-[#2B3674] flex items-center gap-2">
                                            <span className="w-2.5 h-2.5 rounded-full bg-[#EE5D50] inline-block"></span>
                                            Hết hàng:
                                          </span>
                                          <span className="text-[#EE5D50] font-bold">{stats.outOfStock} SP</span>
                                        </div>
                                        <div className="flex justify-between items-center text-sm font-medium">
                                          <span className="text-[#2B3674] flex items-center gap-2">
                                            <span className="w-2.5 h-2.5 rounded-full bg-[#4318FF] inline-block"></span>
                                            Tồn kho tổng:
                                          </span>
                                          <span className="text-[#2B3674] font-bold">{stats.totalStock?.toLocaleString('vi-VN') || 0} thiết bị</span>
                                        </div>
                                      </div>
                                    </div>
                                    ...
                                    <div className="py-4 md:py-0 md:px-6 flex flex-col justify-center">
                                      <h5 className="text-xs font-extrabold text-[#A3AED0] uppercase tracking-wider mb-3">Sản phẩm bán chạy (Top Sellers)</h5>
                                      {stats.topSellers && stats.topSellers.length > 0 ? (
                                        <div className="space-y-2">
                                          {stats.topSellers.map((item, idx) => (
                                            <div key={idx} className="flex items-center gap-2.5">
                                              <div className="w-6 h-6 rounded bg-white border border-[#E0E5F2] flex items-center justify-center overflow-hidden shrink-0">
                                                {item.thumbnailImage ? (
                                                  <img src={item.thumbnailImage} alt={item.name} className="w-full h-full object-cover" />
                                                ) : (
                                                  <ImageIcon className="text-[#A3AED0]" size={12} />
                                                )}
                                              </div>
                                              <span className="truncate text-xs font-bold text-[#2B3674]" title={item.name}>{item.name}</span>
                                            </div>
                                          ))}
                                        </div>
                                      ) : (
                                        <span className="text-xs text-[#A3AED0] italic py-2">Chưa có thông tin bán hàng.</span>
                                      )}
                                    </div>
                                    ...
                                    <div className="pt-4 md:pt-0 md:pl-6 flex flex-col justify-center gap-2">
                                      <button
                                        ...
                                        className="w-full py-2 bg-[#4318FF] hover:bg-[#3911D1] text-white text-xs font-bold rounded-md transition-all flex items-center justify-center gap-1.5 active:scale-[0.98]"
                                      >
                                        <span>Xem toàn bộ {brand.productsCount || 0} sản phẩm</span>
                                      </button>
                                      <button
                                        onClick={() => onRedirectToCreateProduct && onRedirectToCreateProduct(brand.id)}
                                        className="w-full py-2 bg-transparent hover:bg-neutral-100 text-[#4318FF] text-xs font-bold rounded-md transition-all border border-[#E0E5F2] flex items-center justify-center gap-1.5 active:scale-[0.98]"
                                      >
                                        <Plus size={14} />
                                        <span>Thêm sản phẩm</span>
                                      </button>
        ...
        <div className="px-6 py-4 border-t border-[#E0E5F2] flex flex-col sm:flex-row items-center justify-between gap-4 text-sm font-bold text-[#A3AED0]">
          ...
          {totalPages > 1 && (
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-2 bg-[#F4F7FE] text-[#2B3674] rounded-md text-xs font-bold hover:bg-[#E0E5F2] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Trước
              </button>
              {[...Array(totalPages)].map((_, i) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-full text-xs font-bold transition-all ${currentPage === i + 1 ? 'bg-[#4318FF] text-[#FFFFFF] shadow-md' : 'bg-transparent text-[#A3AED0] hover:bg-[#F4F7FE]'}`}
                >
                  {i + 1}
                </button>
              ))}
          ...
          <div className="bg-[#FFFFFF] rounded-md w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-[#E0E5F2] flex justify-between items-center bg-[#F8FAFC]">
              <h3 className="text-xl font-bold text-[#2B3674] flex items-center gap-2">
                {editingBrand ? <Edit size={20} className="text-[#4318FF]" /> : <Plus size={20} className="text-[#4318FF]" />}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 flex flex-col items-center sm:flex-row gap-6 p-4 bg-[#F4F7FE] rounded-md border border-[#E0E5F2] border-dashed">
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    <h4 className="font-bold text-[#2B3674] mb-1">Logo thương hiệu *</h4>
                    <p className="text-xs text-[#A3AED0] mb-3">Chỉ hỗ trợ định dạng WebP, SVG, PNG, JPG/JPEG. Tối đa 2MB.</p>
                ...
                <div className="md:col-span-1 space-y-4">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <label className="block text-sm font-bold text-[#2B3674]">Mã (BrandCode)</label>
                ...
                <div className="md:col-span-1 space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-[#2B3674] mb-2">Trạng thái hoạt động</label>
                    <label className="flex items-center gap-3 p-3 border border-[#E0E5F2] rounded-md bg-[#F8FAFC] cursor-pointer">
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-bold text-[#2B3674] mb-2">Mô tả thương hiệu</label>
                  <textarea
                    rows="3"
      ...
      {toast && (
        <div className={`fixed bottom-5 right-5 z-[200] max-w-sm w-full bg-white rounded-md shadow-xl border p-4 flex items-start gap-3 animate-in fade-in slide-in-from-bottom-5 duration-300 ${
          ...
          <div className="flex-1">
            ...
            {toast.description && (
              <p className="text-[10px] text-[#A3AED0] mt-1 font-medium leading-relaxed">{toast.description}</p>
            )}
          </div>
        </div>
      )}
