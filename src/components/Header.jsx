      ...
      <div className="container-box flex items-center justify-between py-3 h-16 px-4">
        ...
        <div className="flex items-center space-x-3 text-xs shrink-0">
            ...
            {isLoggedIn && userRole === 'admin' && (
              <Link to="/admin" className="flex items-center px-3 py-2 rounded bg-yellow-500 text-gray-900 border border-yellow-400 hover:bg-yellow-400 font-bold transition text-center shadow-sm">
                Trang<br/>Quản trị
              </Link>
            )}
        ...
        <div className="container-box flex items-center justify-center h-12 overflow-x-auto text-sm">
          {BRANDS.map((brand, idx) => (
            <Link 
              ...
              className="flex-1 text-center hover:bg-white hover:text-primary h-full flex items-center justify-center px-2 transition font-medium whitespace-nowrap"
            >
              {brand}
            </Link>
          ))}
